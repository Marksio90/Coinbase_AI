...
class ExecutionService:
    ...
    def __init__(self, gateway: ExchangeGateway | None = None) -> None:
        ...
        self.allowed_callers = {
            caller.strip()
            for caller in os.getenv("EXECUTION_ALLOWED_CALLERS", "orchestrator").split(",")
            if caller.strip()
        }
        cfg = get_config()
        self.execution_enabled = cfg.trading.enable_execution
        self.retry_attempts = int(os.getenv("EXECUTION_MAX_RETRIES", "3"))
        self.retry_base_delay = float(os.getenv("EXECUTION_RETRY_BASE_DELAY_MS", "250")) / 1000.0

    async def execute(self, request: ExecutionRequest, caller: str) -> ExecutionResponse:
        if not self.execution_enabled:
            logger.warning("execution_globally_disabled")
            return ExecutionResponse(
                accepted=False,
                status="REJECTED",
                reason="execution feature flag is disabled",
            )
        ...
        exchange_order_id = await async_retry(
            _submit,
